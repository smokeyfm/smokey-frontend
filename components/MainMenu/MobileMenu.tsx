...
export const MobileMenu = ({ showMenuHeader, onMenuItemClick, menusData }: any) => {
  ...
  const handleClick = useCallback((kp: any, key: any) => {
    if (onMenuItemClick) {
  ...
  const getSubMenuItem = (localMenuData: any[], parentKeyPath: string, level: number) => {
    ...
    const menuItems = isArray ? localMenuData : menusData?.menu_location_listing[0].menu_item_listing;
            ...
            <Fragment key={pathSlug}>
              {
                ...
                >
                  ...
                  {/* <ListItemText primary={item.name.replace("/", "_")} /> */}
