...
const DesktopMenu: React.FC<IDesktopMenuProps> = (props: IDesktopMenuProps) => {
  ...
  return <Container className={pcWrapClassName}>
    {
      menusData.map((item,index)=>(
          <MyMenuItem onMouseLeave={handleMouseLeave} isActive={currentKey==item.key} key={index} onMouseEnter={handleMouseEnter.bind(null,item)}>
            {item.name}
          </MyMenuItem>
      ))
    }
    {
      menusData.map((item,index)=>(
          <DropDown onMouseLeave={handleMouseLeave} onMouseEnter={handleMouseEnter.bind(null,item)} isActive={currentKey==item.key} key={index}>
            {item.pcMenuItem}
          </DropDown>
      ))
    }
